...
const TarjetaSolicitud = ({ solicitud, cedulaPosgrados, tipo, onActualizar }) => {
    ...
    const handleAprobarPosgrados = async () => {
        setAprobando(true);
        try {
            await solicitudesApi.aprobarPosgrados(solicitud.id, cedulaPosgrados);
            onActualizar();
        } catch (e) {
    ...
    const puedeAprobarPosgrados = tipo === 'terminacion' && solicitud.estado === 'APROBADA_DIRECTOR';
    const puedeGenerarCert = tipo === 'terminacion' && solicitud.estado === 'APROBADA';
    ...
    const handleDescargarCertificado = async () => {
        ...
        try {
            const { blob } = await solicitudesApi.descargarCertificadoPdf(solicitud.id, cedulaPosgrados);
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `certificado-terminacion-${solicitud.id}.pdf`;
            a.click();
            URL.revokeObjectURL(url);
        } catch (e) {
            setErrorDescarga('No se pudo generar el certificado.');
        } finally {
    ...
    return (
        <>
            <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm hover:shadow-md transition-shadow">
                ...
                {errorDescarga && (
                    <p className="mb-2 text-xs text-red-600">{errorDescarga}</p>
                )}

                {/* Acciones */}
                <div className="flex flex-wrap gap-2 mt-1">
                    {/* Botón aprobar posgrados */}
                    {puedeAprobarPosgrados && (
                        <button onClick={handleAprobarPosgrados} disabled={aprobando}
                    ...
                    {puedeGenerarCert && (
                        <button onClick={handleDescargarCertificado} disabled={descargando}
                            ...
                            {descargando ? 'Generando…' : 'Generar certificado'}
                        </button>
                    )}
                    ...
                    {puedeRechazar && (
                        <button onClick={() => setMostrarRechazo(true)}
                            className="inline-flex items-center gap-1.5 rounded-xl border border-red-200 bg-red-50 px-4 py-2 text-xs font-semibold text-red-700 hover:bg-red-100">
                            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
